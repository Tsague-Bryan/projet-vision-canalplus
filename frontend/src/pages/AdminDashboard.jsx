...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-gray-900 text-white text-xs px-3 py-2 rounded-xl shadow-xl border border-gray-700">
      ...
      {payload.map((p, i) => (
        <p key={i} style={{ color: p.color }} className="font-bold">{Number(p.value).toLocaleString()}</p>
      ))}
    </div>
  );
};
...
const ActionBtn = ({ onClick, color = "gray", icon, title, label }) => (
  <button onClick={onClick} title={title}
    className={`flex items-center gap-1 px-2 py-1 rounded-lg border text-xs font-semibold transition-colors ${colorMap[color]}`}>
...
export default function AdminDashboard() {
  ...
  // ✅ States manquants pour les décodeurs
...
const [addDecoderError, setAddDecoderError] = useState("");
  ...
  // ✅ States pour les demandes de recharge
  ...
  const emptyPartner = {
    name:"", prenom:"", structure:"", pays:"", ville:"",
    quartier:"", telephone:"", email:"", password:"", codePromo:"", wallet_balance:""
  };
  const [newPartner, setNewPartner] = useState(emptyPartner);
  const [editData, setEditData]     = useState({
    name:"", email:"", structure:"", pays:"", ville:"",
    quartier:"", telephone:"", codePromo:""
  });

  // ✅ Socket.IO : connexion admin pour recevoir les notifications en temps réel
  useEffect(() => {
    const socket = io(SOCKET_URL, { transports: ["websocket", "polling"] });
    socket.on("new_notification", (data) => {
      console.log("🔔 Notification reçue :", data);
      // Si on est sur la page recharges, rafraîchir automatiquement
      if (data.type === "recharge") fetchRecharges();
    });
  ...
  // ✅ URLs corrigées pour correspondre aux routes backend existantes
  const fetchStats = useCallback(async () => {
    try {
      ...
      const chart = (res.data.reabonnementsMois || []).map(r => ({
        month: moisNoms[(r.mois || 1) - 1],
        abonnements: r.total,
      }));
      setChartData(chart);
 ...
 const fetchDecoders = useCallback(async () => {
  ...
  try {
    const res = await axios.get(`http://localhost:5000/api/decodeurs/all`); // URL complète explicite
...
const addDecoder = async () => {
  setAddDecoderError("");
  if (!newDecoder.numero.trim()) return setAddDecoderError("Le numéro de décodeur est requis.");
  if (!newDecoder.partner_id)    return setAddDecoderError("Veuillez sélectionner un partenaire.");
  try {
    await axios.post(`${API}/decodeurs`, {
      numero:     newDecoder.numero.trim(),
      partner_id: Number(newDecoder.partner_id),
    });
    setShowAddDecoderModal(false);
    setNewDecoder(emptyDecoder);
    fetchDecoders();
  } catch (e) {
    const msg = e.response?.data?.message || e.message;
    setAddDecoderError(msg);
  }
};

  // ✅ Fetch demandes de recharge
  ...
  useEffect(() => {
    ...
    fetchDecoders();
    fetchRecharges();
  }, [fetchPartners, fetchStats, fetchDecoders, fetchRecharges]);

  const approvePartner = async (id) => {
    await axios.put(`${API}/partners/${id}/approve`);
    fetchPartners();
  };
  const rejectPartner = async (id) => {
    await axios.put(`${API}/partners/${id}/reject`);
    fetchPartners();
  };
  ...
  const openEdit = (p) => {
    setEditId(p.id);
    setEditData({ name:p.name, email:p.email, structure:p.structure,
      pays:p.pays, ville:p.ville, quartier:p.quartier,
      telephone:p.telephone, codePromo:p.codePromo });
    setShowEditModal(true);
  };
  const saveEdit = async () => {
    await axios.put(`${API}/partners/${editId}`, editData);
    setShowEditModal(false);
    fetchPartners();
  };
  const validateReabonnement = async (id) => {
    try {
      const res = await axios.post(`${API}/partners/${id}/validate-reabonnement`, { formulePrix: 10000 });
  ...
  const creditWallet = async (id) => {
    ...
    if (!amount || isNaN(amount) || Number(amount) <= 0) return alert("Montant invalide");
    try {
      const res = await axios.post(`${API}/partners/${id}/credit`, { amount: Number(amount) });
      alert(`Crédité ! Solde : ${res.data.wallet_balance} FCFA`);
      fetchPartners();
    } catch (e) { alert("Erreur crédit"); }
  };

  // ✅ Valider / Rejeter une demande de recharge
  const validerRecharge = async (id) => {
    if (!window.confirm("Valider cette demande et créditer le partenaire ?")) return;
    try {
      await axios.post(`${API}/admin/recharges/${id}/valider`);
      alert("Recharge validée ! Le portefeuille a été crédité.");
      fetchRecharges();
      fetchPartners();
  ...
  const rejeterRecharge = async (id) => {
    ...
    try {
      await axios.post(`${API}/admin/recharges/${id}/rejeter`);
      fetchRecharges();
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 pt-5 pb-2 border-b border-gray-50">
            <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest">Tendance</p>
            <h2 className="text-sm font-bold text-gray-900 mt-0.5">Abonnements mensuels</h2>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-5 pt-5 pb-2 border-b border-gray-50">
            <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest">Statuts</p>
            <h2 className="text-sm font-bold text-gray-900 mt-0.5">Répartition partenaires</h2>
          </div>
          <div className="px-4 pb-5 pt-3">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={[
              ...
              ]} margin={{ top:5, right:10, left:0, bottom:5 }} barSize={40}>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 pt-5 pb-3 border-b border-gray-50 flex items-center justify-between">
          <div>
            <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest">Récents</p>
            <h2 className="text-sm font-bold text-gray-900 mt-0.5">Derniers partenaires inscrits</h2>
          </div>
          <button onClick={() => setActivePage("partners")}
            className="text-xs font-semibold text-red-600 hover:text-red-700 transition-colors">Voir tout →</button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50">
                {["Partenaire","Ville","Code Promo","Portefeuille","Statut"].map(h => (
            ...
            <tbody>
              {partners.slice(0, 5).map((p, i) => (
                <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? "border-t border-gray-50" : ""}`}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-red-100 text-red-600 font-bold text-xs flex items-center justify-center flex-shrink-0">
                        {(p.name?.[0] || "?").toUpperCase()}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900 text-sm">{p.name} {p.prenom}</p>
      ...
      {/* Aperçu recharges en attente */}
      {recharges.filter(r => r.statut === "en_attente").length > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-bold text-amber-800">
              🔔 {recharges.filter(r => r.statut === "en_attente").length} demande(s) de recharge en attente
            </p>
            <button onClick={() => setActivePage("recharges")}
              className="text-xs font-semibold text-amber-700 hover:text-amber-900">Voir tout →</button>
          </div>
        </div>
      )}
    </div>
  ...
  // ✅ renderDecoders avec les bons states
const renderDecoders = () => (
  <div className="flex flex-col gap-5">
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-5 pt-5 pb-3 border-b border-gray-50 flex items-center justify-between">
        ...
        <button
          onClick={() => { setAddDecoderError(""); setNewDecoder(emptyDecoder); setShowAddDecoderModal(true); }}
          ...
          <Icon name="plus" size={15} /> Ajouter un décodeur
        </button>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b">
              <th className="px-5 py-3 text-left text-xs text-gray-400 uppercase">Numéro</th>
              <th className="px-5 py-3 text-left text-xs text-gray-400 uppercase">Statut</th>
              <th className="px-5 py-3 text-left text-xs text-gray-400 uppercase">Partenaire attribué</th>
            </tr>
          </thead>
          <tbody>
            {loadingDecoders ? (
              ...
              <tr><td colSpan={3} className="text-center py-10 text-gray-400">Aucun décodeur enregistré</td></tr>
            ) : decodeurs.map((d) => (
              <tr key={d.id} className="border-t hover:bg-gray-50">
                <td className="px-5 py-3 font-mono font-semibold text-gray-900">{d.numero}</td>
                <td className="px-5 py-3">
                  <span className={`px-2 py-1 text-xs rounded font-semibold ${d.status === "free" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                ...
                <td className="px-5 py-3">
                  {d.name ? (
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-red-100 text-red-600 font-bold text-xs flex items-center justify-center flex-shrink-0">
                        {(d.name[0] || "?").toUpperCase()}
                      </div>
                      <span className="text-gray-700 font-medium">{d.prenom} {d.name}</span>
                    </div>
                  ) : (
                    <span className="text-gray-400 text-xs italic">Non attribué</span>
                  )}
                </td>
              </tr>
            ))}
      ...
      <Modal title="Ajouter un décodeur" onClose={() => setShowAddDecoderModal(false)}>
        <div className="flex flex-col gap-4">
          ...
          <Field
            ...
            placeholder="Ex: 4512001234"
            value={newDecoder.numero}
            onChange={e => setNewDecoder({ ...newDecoder, numero: e.target.value })}
          />

          {/* Partenaire — obligatoire selon le backend */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Partenaire <span className="text-red-500">*</span>
            </label>
            <select
              value={newDecoder.partner_id}
              onChange={e => setNewDecoder({ ...newDecoder, partner_id: e.target.value })}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-900
                         focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent
              ...
              {partners
                ...
                .map(p => (
                  <option key={p.id} value={p.id}>
                    {p.prenom} {p.name}{p.codePromo ? ` — ${p.codePromo}` : ""}
                  </option>
                ))}
            </select>
            <p className="text-[11px] text-gray-400 mt-1">Seuls les partenaires validés sont listés.</p>
          </div>
        ...
        <div className="flex gap-3 mt-6">
          <button
            onClick={() => setShowAddDecoderModal(false)}
  ...
  // ✅ Page Recharges : voir et traiter les demandes de recharge
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                {["Partenaire","Opérateur","N° Transaction","Montant","Capture","Statut","Actions"].map(h => (
            ...
            <tbody>
              {loadingRecharges ? (
                ...
                <tr><td colSpan={7} className="text-center py-10 text-gray-400">Aucune demande</td></tr>
              ) : recharges.map((r, i) => (
                <tr key={r.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? "border-t border-gray-50" : ""}`}>
                  <td className="px-4 py-3">
                    <p className="font-semibold text-gray-900">{r.prenom} {r.name}</p>
                  ...
                  <td className="px-4 py-3">
                    {r.capture ? (
                      <a href={`http://localhost:5000/uploads/recharges/${r.capture}`} target="_blank" rel="noreferrer"
                        className="text-xs text-blue-600 underline hover:text-blue-800">Voir</a>
                  ...
                  <td className="px-4 py-3">
                    {r.statut === "en_attente" && (
                      <div className="flex gap-1.5">
                        <ActionBtn onClick={() => validerRecharge(r.id)} color="green" icon="check" label="Valider" />
                        <ActionBtn onClick={() => rejeterRecharge(r.id)} color="red"   icon="x"     label="Rejeter" />
                      </div>
                    )}
                  </td>
                </tr>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-52">
          ...
          <input type="text" placeholder="Rechercher un partenaire…" value={search}
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 bg-white" />
        </div>
        <select value={filter} onChange={e => { setFilter(e.target.value); setCurrentPage(1); }}
          className="px-4 py-2.5 rounded-xl border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-red-500 text-gray-700">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                {["Partenaire","Structure","Localisation","Téléphone","Code Promo","Portefeuille","Statut","Actions"].map(h => (
                  <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide whitespace-nowrap">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="text-center py-12 text-gray-400">Aucun partenaire trouvé</td></tr>
              ) : paginated.map((p, i) => (
                <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? "border-t border-gray-50" : ""}`}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900">{p.name} {p.prenom}</p>
                        <p className="text-xs text-gray-400">{p.email}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <ActionBtn onClick={() => approvePartner(p.id)}       color="green"  icon="check"      title="Valider" />
                      <ActionBtn onClick={() => rejectPartner(p.id)}        color="red"    icon="x"          title="Rejeter" />
                      <ActionBtn onClick={() => openEdit(p)}                color="blue"   icon="edit"       title="Modifier" />
                      <ActionBtn onClick={() => deletePartner(p.id)}        color="gray"   icon="trash"      title="Supprimer" />
                      <ActionBtn onClick={() => validateReabonnement(p.id)} color="indigo" icon="refresh"    title="Réabonnement" label="Réab." />
                      <ActionBtn onClick={() => creditWallet(p.id)}         color="purple" icon="creditcard" title="Créditer" label="Crédit" />
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="flex items-center justify-between px-5 py-4 border-t border-gray-50">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}
                className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed">
                <Icon name="chevronL" size={14} />
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(n => (
                <button key={n} onClick={() => setCurrentPage(n)}
                  className={`w-8 h-8 rounded-xl text-xs font-semibold transition-colors ${n === currentPage ? "bg-red-600 text-white" : "border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                  {n}
                </button>
              ))}
  ...
  const renderStats = () => (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 pt-5 pb-2 border-b border-gray-50">
          <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest">Évolution</p>
          <h2 className="text-sm font-bold text-gray-900 mt-0.5">Abonnements mensuels</h2>
        </div>
  ...
  const renderWallets = () => (
    <div className="flex flex-col gap-5">
      <KpiCard label="Total portefeuilles" value={`${totalWallet.toLocaleString()} FCFA`}
        sub={`Réparti sur ${approved} partenaires validés`}
        color="bg-green-600 text-white" icon="wallet" />
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-5 pt-5 pb-3 border-b border-gray-50">
          <p className="text-[11px] text-gray-400 font-semibold uppercase tracking-widest">Soldes</p>
          <h2 className="text-sm font-bold text-gray-900 mt-0.5">Portefeuilles partenaires</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50">
                {["Partenaire","Code Promo","Statut","Solde","Action"].map(h => (
                  <th key={h} className="px-5 py-3 text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {partners.map((p, i) => (
                <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? "border-t border-gray-50" : ""}`}>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-green-100 text-green-700 font-bold text-xs flex items-center justify-center">
                        {(p.name?.[0] || "?").toUpperCase()}
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900">{p.name} {p.prenom}</p>
                  ...
                  <td className="px-5 py-3"><StatusBadge status={p.status} /></td>
                  <td className="px-5 py-3">
                    <span className="text-lg font-bold text-green-700">{(Number(p.wallet_balance) || 0).toLocaleString()}</span>
                    <span className="text-xs text-gray-400 ml-1">FCFA</span>
                  </td>
                  <td className="px-5 py-3">
                    <ActionBtn onClick={() => creditWallet(p.id)} color="green" icon="creditcard" label="Créditer" title="Créditer le portefeuille" />
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  const pageMap    = { dashboard: renderDashboard, partners: renderPartners, stats: renderStats, wallets: renderWallets, recharges: renderRecharges, decoders: renderDecoders };
  const pageTitles = { dashboard:"Tableau de bord", partners:"Partenaires", stats:"Statistiques", wallets:"Portefeuilles", recharges:"Demandes de recharge", decoders:"Décodeurs" };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <aside className={`flex flex-col bg-gray-900 text-white transition-all duration-300 flex-shrink-0
                         ${sidebarOpen ? "w-64" : "w-16"} min-h-screen sticky top-0 z-30`}>
        <div className={`flex items-center gap-3 px-4 py-5 border-b border-gray-800 ${sidebarOpen ? "" : "justify-center"}`}>
          <div className="w-9 h-9 rounded-xl bg-red-600 flex items-center justify-center flex-shrink-0 shadow-md">
            <span className="text-white font-black text-sm">VC</span>
          </div>
          {sidebarOpen && (
            <div>
              <p className="text-sm font-bold leading-none">Vision Canal<span className="text-red-500">+</span></p>
        ...
        <nav className="flex flex-col gap-1 p-3 flex-1">
          {navItems.map(({ id, label, icon }) => {
            ...
            const badge = id === "recharges" ? recharges.filter(r => r.statut === "en_attente").length : 0;
            return (
              <button key={id} onClick={() => setActivePage(id)} title={!sidebarOpen ? label : undefined}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 text-left
                             ${active ? "bg-red-600 text-white shadow-md" : "text-gray-400 hover:bg-gray-800 hover:text-white"}
                ...
                {sidebarOpen && active && badge === 0 && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-white opacity-70" />}
              </button>
            );
          })}
        </nav>
          ...
          {sidebarOpen && (
            <div className="flex items-center gap-3 px-3 py-2 mb-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-gray-400">Vision Canal+</p>
              </div>
            </div>
          )}
          <button onClick={handleLogout}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium
                        text-gray-400 hover:bg-gray-800 hover:text-red-400 w-full transition-all
                        ${!sidebarOpen ? "justify-center" : ""}`}
      ...
      {showAddModal && (
        <Modal title="Ajouter un partenaire" onClose={() => setShowAddModal(false)}>
          <div className="grid grid-cols-2 gap-4">
            {[
              { label:"Nom", key:"name" }, { label:"Prénom", key:"prenom" },
              { label:"Email", key:"email", type:"email" }, { label:"Mot de passe", key:"password", type:"password" },
              { label:"Structure", key:"structure" }, { label:"Pays", key:"pays" },
              { label:"Ville", key:"ville" }, { label:"Quartier", key:"quartier" },
              { label:"Téléphone", key:"telephone" }, { label:"Code Promo", key:"codePromo" },
            ].map(({ label, key, type }) => (
              <Field key={key} label={label} type={type} value={newPartner[key]}
                onChange={e => setNewPartner({ ...newPartner, [key]: e.target.value })} />
            ))}
          </div>
          <div className="flex gap-3 mt-6">
            <button onClick={() => setShowAddModal(false)}
              className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50">Annuler</button>
            <button onClick={addPartner}
              className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold">Ajouter</button>
          </div>
        </Modal>
      )}
      ...
      {showEditModal && (
        <Modal title="Modifier le partenaire" onClose={() => setShowEditModal(false)}>
          <div className="grid grid-cols-2 gap-4">
            {[
              { label:"Nom", key:"name" }, { label:"Email", key:"email", type:"email" },
              { label:"Structure", key:"structure" }, { label:"Pays", key:"pays" },
              { label:"Ville", key:"ville" }, { label:"Quartier", key:"quartier" },
              { label:"Téléphone", key:"telephone" }, { label:"Code Promo", key:"codePromo" },
            ].map(({ label, key, type }) => (
              <Field key={key} label={label} type={type} value={editData[key]}
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button onClick={saveEdit}
              className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold">Enregistrer</button>
          </div>
        </Modal>
      )}
    </div>
